...
class SpatialTemporalPromptReasoner(nn.Module):
    def __init__(self,
        ...
        super(SpatialTemporalPromptReasoner, self).__init__()
        ...
        self.prompt_reasoning = prompt_reasoning
        ...
        nn.init.constant_(self.prompt_cls[-1].bias, bias_init)

    def forward(self, track_instances, prompt=None, prompt_mask=None):
        ...
        # 4. Prompt reasoning
        if self.prompt_reasoning:
            track_instances = self.forward_prompt_reasoning(track_instances)
            track_instances = self.forward_prompt_prediction(track_instances)
    ...
    def forward_prompt_reasoning(self, track_instances):
        ...
        valid_idxes = (track_instances.hist_padding_masks[:, -1] == 0)
        embed = track_instances.cache_query_feats[valid_idxes]
        track_instances.prompt_embeds = embed.clone()

        return track_instances

    def forward_prompt_prediction(self, track_instances):
        """Predict whether is referent objects"""
        prompt_predictions = self.prompt_cls(track_instances.prompt_embeds)
    ...
    def init_params_and_layers(self):
        ...
        # Modules for prompt reasoning
        if self.prompt_reasoning:
            # classification refinement from prompt
            cls_branch_prompt = []
            for _ in range(self.num_reg_fcs):
                cls_branch_prompt.append(Linear(self.embed_dims, self.embed_dims))
                cls_branch_prompt.append(nn.LayerNorm(self.embed_dims))
                cls_branch_prompt.append(nn.ReLU(inplace=True))
            cls_branch_prompt.append(Linear(self.embed_dims, 1))
            self.prompt_cls = nn.Sequential(*cls_branch_prompt)
    ...
    def frame_shift(self, track_instances: Instances):
        ...
        track_instances.motion_predictions = torch.cat((
            track_instances.motion_predictions[:, 1:, :],
            torch.zeros_like(track_instances.motion_predictions[:, 0:1, :])), dim=1)
        ...
        track_instances.fut_padding_masks = torch.cat((
            track_instances.fut_padding_masks[:, 1:],
            torch.ones_like(track_instances.fut_padding_masks[:, 0:1]).bool()), dim=1)
        ...
        track_instances.fut_position_embeds = torch.cat((
            track_instances.fut_position_embeds[:, 1:, :],
            torch.zeros_like(track_instances.fut_position_embeds[:, 0:1, :])), dim=1)
