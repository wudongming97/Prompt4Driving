...
# ------------------------------------------------------------------------
# Modified from PF-Track (https://github.com/TRI-ML/PF-Track)
# Copyright (c) 2023 toyota research instutute.
# ------------------------------------------------------------------------
import torch
...
from .spatial_temporal_prompt_reason import SpatialTemporalPromptReasoner
from .utils import time_position_embedding, xyz_ego_transformation, normalize, denormalize
from transformers import RobertaModel, RobertaTokenizerFast
from ..utils import VisionLanguageFusionModule, FeatureResizer, PositionEmbeddingSine1D


@DETECTORS.register_module()
class PromptCam3DTracker(MVXTwoStageDetector):
    def __init__(self,
                 num_classes=10,
                 num_query=100,
                 tracking=True,
                 ...
                 test_cfg=None,
                 pretrained=None):
        super(PromptCam3DTracker, self).__init__(pts_voxel_layer, pts_voxel_encoder,
        ...
        self.STReasoner = SpatialTemporalPromptReasoner(**spatial_temporal_reason)
        ...
        # Cross-modal fusion
        self.fusion_module = VisionLanguageFusionModule(d_model=self.embed_dims, nhead=8, cross_attn=cross_modal_neck)
        self.text_pos = PositionEmbeddingSine1D(self.embed_dims, normalize=True)
    ...
    def generate_empty_instance(self):
        ...
        """Prompt Reasoning"""
        track_instances.prompt_embeds = torch.zeros(
            (len(track_instances), self.embed_dims), dtype=torch.float32, device=device)
        track_instances.prompt_preds = torch.zeros(
    ...
    def frame_summarization(self, track_instances, tracking=False):
        ...
        if self.STReasoner.future_reasoning:
            ...
            track_instances.fut_xyz[active_mask] = track_instances.reference_points[active_mask].clone()[:, None,
                                                   :].repeat(1, self.fut_len, 1)
            track_instances.fut_bboxes[active_mask] = track_instances.bboxes[active_mask].clone()[:, None, :].repeat(1,
                                                                                                                     self.fut_len,
                                                                                                                     1)
    ...
    def forward_train(self,
        ...
        # Extract the features of prompt
        prompt_features, prompt_pad_mask = self.extract_prompt_feats([img_metas[0]['prompt'][0]], img.device)

        # Cross-modal fusion
        prompt_pos = self.text_pos(prompt_features, prompt_pad_mask).permute(2, 0, 1)
        cross_img_feats = list()
        for img_feat in img_feats:
            h, w = img_feat[0].shape[-2:]
            img_feat = img_feat[0].flatten(0, 1).flatten(2, 3).permute(2, 0, 1)
            cross_img_feat = self.fusion_module(tgt=img_feat,
                                                memory=prompt_features.permute(1, 0, 2).repeat(1, num_cam, 1),
                                                memory_key_padding_mask=prompt_pad_mask.repeat(num_cam, 1),
                                                pos=prompt_pos.repeat(1, num_cam, 1),
                                                query_pos=None
                                                )
            cross_img_feat = cross_img_feat.permute(1, 2, 0).view(num_cam, self.embed_dims, h, w).unsqueeze(0)
            cross_img_feats.append([cross_img_feat])
        img_feats = cross_img_feats
        ...
        for frame_idx in range(num_frame):
            ...
            # 3. Spatial-temporal reasoning and prompt reasoning
            track_instances = self.STReasoner(track_instances, prompt_features, prompt_pad_mask)
            ...
            out['loss_dict'] = self.criterion.loss_prompt(frame_idx,
                                                          out['loss_dict'],
                                                          img_metas[0]['prompt_object_ids'][frame_idx],
                                                          ff_instance_ids[frame_idx],
                                                          track_instances)
            ...
            if frame_idx < num_frame - 1:
                active_track_instances = self.STReasoner.sync_pos_embedding(active_track_instances,
    ...
    def forward_track(self,
        ...
        if self.runtime_tracker.timestamp is None or abs(timestamp[0] - self.runtime_tracker.timestamp) > 10 \
                or img_metas[0]['prompt'][0] != self.runtime_tracker.prompt:
            ...
            self.runtime_tracker.prompt = img_metas[0]['prompt'][0]
        ...
        for frame_idx in range(num_frame):
            ...
            if prev_active_track_instances is None:
                ...
                prev_active_track_instances = self.STReasoner.sync_pos_embedding(prev_active_track_instances,
                                                                                 self.query_embedding)
            ...
            self.STReasoner(track_instances, prompt_features, prompt_pad_mask)
            ...
            out['prompt_preds'] = track_instances.prompt_preds
            ...
            out['prompt_preds'] = out['prompt_preds'][related_cat_mask, :].sigmoid()
        ...
        bbox_results = [
            track_bbox3d2result(bboxes, scores, labels, obj_idxes, track_scores, forecasting, prompt_pred=prompt_preds)
            for bboxes, scores, labels, obj_idxes, track_scores, forecasting, prompt_preds in bbox_list
        ]
        bbox_results[0]['track_ids'] = [f'{self.runtime_tracker.current_seq}-{i}' for i in
                                        bbox_results[0]['track_ids'].long().cpu().numpy().tolist()]
        return bbox_results

    def extract_prompt_feats(self, prompt, device):
        ''' Extract the features of language prompt
        '''
        # print(prompt)
        tokenized_prompt = self.tokenizer.batch_encode_plus(prompt, padding="longest", return_tensors='pt').to(device)
        # with torch.inference_mode(mode=self.freeze_text_encoder):
        encoded_prompt = self.prompt_encoder(**tokenized_prompt)
        # Transpose memory because pytorch's attention expects sequence first
        prompt_features = encoded_prompt.last_hidden_state.clone()
        prompt_features = self.prompt_proj(prompt_features)  # change text embeddings dim to model dim
        # Invert attention mask that we get from huggingface because its the opposite in pytorch transformer
        prompt_pad_mask = tokenized_prompt.attention_mask.ne(1).bool()  # [B, S]

        # text_sentence_features = text_features
        return prompt_features, prompt_pad_mask
    ...
    def init_params_and_layers(self):
        ...
        self.prompt_encoder = RobertaModel.from_pretrained('./roberta_base/', local_files_only=True)
        self.prompt_proj = FeatureResizer(
            input_feat_size=self.prompt_encoder.config.hidden_size,
            output_feat_size=self.embed_dims,
            dropout=True,
        )
...
def track_bbox3d2result(bboxes, scores, labels, obj_idxes, track_scores, forecasting=None, attrs=None, prompt_pred=None):
    ...
    if prompt_pred is not None:
        result_dict['prompt_pred'] = prompt_pred.cpu()
