...
from nuscenes.eval.tracking.data_classes import TrackingMetrics, TrackingMetricDataList, TrackingConfig, \
    TrackingMetricData
from nuscenes.eval.tracking.render import recall_metric_curve, summary_plot

from projects.tracking_plugin.datasets.utils.nuprompt_eval_tools import load_gt_prompt, add_center_dist_prompt, filter_eval_boxes_prompt, \
    create_tracks_prompt, TrackingBox


class PromptTrackingEval:
    """
    ...
    def __init__(self,
                 ...
                 filter_gt_from_nuprompt: bool = True,):
        """
        ...
        self.filter_gt_from_nuprompt = filter_gt_from_nuprompt
        ...
        pred_boxes, self.meta = load_prediction(self.result_path, self.cfg.max_boxes_per_sample, TrackingBox,
                                                verbose=verbose)
        # Load GT, where we perform filtering according to prompt.
        gt_boxes = load_gt_prompt(nusc, self.result_path, self.eval_set, TrackingBox, verbose=verbose,
        ...
        pred_boxes = add_center_dist_prompt(nusc, pred_boxes)
        gt_boxes = add_center_dist_prompt(nusc, gt_boxes)

        # Filter boxes (distance, points per box, etc.).
        if verbose:
            print('Filtering tracks')
        pred_boxes = filter_eval_boxes_prompt(nusc, pred_boxes, self.cfg.class_range, verbose=verbose)
        if verbose:
            print('Filtering ground truth tracks')
        gt_boxes = filter_eval_boxes_prompt(nusc, gt_boxes, self.cfg.class_range, verbose=verbose)

        # Convert boxes to tracks format.
        self.tracks_gt = create_tracks_prompt(gt_boxes, nusc, self.eval_set, gt=True)
        self.tracks_pred = create_tracks_prompt(pred_boxes, nusc, self.eval_set, gt=False)
    ...
    def evaluate(self) -> Tuple[TrackingMetrics, TrackingMetricDataList]:
        ...
        def accumulate_class(curr_class_name):
            curr_ev = TrackingEvaluation(self.tracks_gt[curr_class_name], self.tracks_pred[curr_class_name], curr_class_name, self.cfg.dist_fcn_callable,
...
if __name__ == "__main__":
    ...
    nusc_eval = PromptTrackingEval(config=cfg_, result_path=result_path_, eval_set=eval_set_, output_dir=output_dir_,
