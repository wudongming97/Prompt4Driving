...
train_pipeline_multiframe = [
    ...
    dict(type='Collect3D', keys=['points', 'gt_bboxes_3d', 'gt_labels_3d', 'instance_inds', 'img', 'timestamp',
                                 'l2g_r_mat', 'l2g_t', 'l2g', 'gt_forecasting_locs', 'gt_forecasting_masks',
                                 'gt_forecasting_types'],
         meta_keys=['filename', 'img_shape', 'ori_shape', 'lidar2img', 'pad_shape', 'scale_factor', 'box_mode_3d',
                    'box_type_3d', 'img_norm_cfg', 'sample_idx', 'pts_filename', 'prompt', 'prompt_object_ids']),
]
...
test_pipeline_multiframe = [
    ...
    dict(type='Collect3D', keys=['img', 'timestamp', 'l2g_r_mat', 'l2g_t', 'l2g'],
         meta_keys=['filename', 'img_shape', 'ori_shape', 'lidar2img', 'pad_shape', 'scale_factor', 'box_mode_3d',
                    'box_type_3d', 'img_norm_cfg', 'sample_idx', 'pts_filename', 'prompt', 'scene_token']
         )
]
...
data = dict(
    ...
    test_tracking=dict(type=dataset_type, pipeline=test_pipeline,
                       ...
                       data_root=data_root, test_mode=True,
                       classes=class_names, modality=input_modality,
                       ann_file=data_root + 'nuprompt_infos_val.pkl',
                       num_frames_per_sample=1, ))

model = dict(
    type='PromptCam3DTracker',
    tracking=True,
    train_backbone=True,
    use_grid_mask=True,
    ...
    spatial_temporal_reason=dict(  # past and future reasoning
        ...
        prompt_reasoning=True,  # use prompt reasoning
        hist_len=3,
        fut_len=8,
        pc_range=point_cloud_range,
    ...
    img_neck=dict(
        ...
        out_channels=256,
        num_outs=2),
    cross_modal_neck='MultiheadFlashAttention',
    pts_bbox_head=dict(
...
total_epochs = 12
evaluation = dict(interval=37, pipeline=test_pipeline)
